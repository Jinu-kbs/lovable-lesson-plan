...
| **AI 모델** | Claude | Gemini, Claude, GPT | 멀티 모델 선택 | Gemini | **Claude Opus 4.8(기본), Sonnet 4.6·Haiku 4.5** |
...
| 모델 | 특징 | 용도 |
|------|------|------|
| **Claude Opus 4.8**(기본) | 최고 성능, 복잡한 추론·장기 에이전트 코딩 | 아키텍처 설계, 대규모 리팩토링 |
| **Claude Sonnet 4.6**(보조) | 빠르고 정확 | 일반 개발 작업 |
| **Claude Haiku 4.5**(보조) | 초고속 응답 | 코드 포맷팅, 간단한 수정 |

> 💡 **팁**: 기본 모델은 Opus 4.8입니다. 일반 코딩은 Sonnet 4.6으로 전환하면 비용을 아낄 수 있으며, 모델 전환은 `/model` 명령으로 가능합니다.
