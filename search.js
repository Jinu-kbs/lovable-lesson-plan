// search.js — 사이트 전체 교안 검색 (클라이언트 사이드, lessons-data.js 기반)
// 홈(index.html)의 구글형 검색창 자동완성 + 검색 결과 페이지(search.html)를 함께 구동한다.
(function () {
  'use strict';
  ...
  function buildIndex() {
    ...
    if (typeof LESSONS !== 'undefined' && LESSONS.tools) {
      ...
      (LESSONS.special || []).forEach(function (s) {
        idx.push({
          title: s.title, desc: '', url: s.url, icon: s.icon, color: s.color,
          badge: '심화·리포트', kw: (s.id || '').toLowerCase()
        });
      });
    }
  ...
  function scoreItem(it, q, synTerm) {
    ...
    var hay = (it.title + ' ' + it.desc + ' ' + it.badge + ' ' + it.kw).toLowerCase();
    var s = 0;
    if (title.indexOf(q) === 0) s = 100;
    else if (title.indexOf(q) !== -1) s = 60;
    else if (hay.indexOf(q) !== -1) s = 30;
    else if (synTerm && hay.indexOf(synTerm) !== -1) s = 15;
    return s;
  }
  ...
  window.searchLessons = function (query) {
    ...
    return buildIndex()
      .map(function (it) { return { it: it, sc: scoreItem(it, q, synTerm) }; })
    ...
    input.addEventListener('input', function () {
      ...
      box.innerHTML = results.map(function (r) {
        return '<a class="hs-item" href="' + esc(r.url) + '">' +
          ...
          '<span class="hs-text"><span class="hs-title">' + esc(r.title) + '</span>' +
          '<span class="hs-badge">' + esc(r.badge) + '</span></span></a>';
  ...
  function initSearchPage() {
    ...
    function render(q) {
      q = (q || '').trim();
      if (!q) {
        if (countEl) countEl.textContent = '검색어를 입력하세요.';
        results.innerHTML = '<p class="search-hint">도구 이름(러버블·커서·Claude Code…), 주제(터미널·배포·프롬프트…), 수준으로 검색해 보세요.</p>';
        return;
      }
      var list = window.searchLessons(q);
      if (countEl) countEl.textContent = '"' + q + '" 검색 결과 ' + list.length + '건';
      if (!list.length) {
        results.innerHTML = '<p class="search-hint">"' + esc(q) + '"에 대한 결과가 없습니다. 다른 검색어를 시도해 보세요.</p>';
        return;
      }
      results.innerHTML = list.map(function (r) {
        return '<a class="search-result" href="' + esc(r.url) + '">' +
          ...
          '<span class="sr-body"><span class="sr-title">' + esc(r.title) + '</span>' +
          (r.desc ? '<span class="sr-desc">' + esc(r.desc) + '</span>' : '') +
    ...
    if (input) {
      ...
      input.addEventListener('input', function () {
        render(input.value);
        var u = input.value.trim() ? ('?q=' + encodeURIComponent(input.value.trim())) : location.pathname;
