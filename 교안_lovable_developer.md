...
> **하이브리드 워크플로우 실전 사례**: "러버블에서 교안 작성 → HTML로 변환 → 깃허브 자동 연동 → 안티그래비티에서 에이전트로 코드 리팩토링·테스트 (멀티 에이전트 병렬 작업이 신세계) → 4시간마다 자동 깃허브 업데이트 설정" — 실사용자 워크플로우. 러버블로 큰 틀을 잡고, 안티그래비티나 커서에서 본격 개발하는 방식이 실전에서 효과적입니다.
...
안티그래비티(Google Antigravity)는 데스크톱 에이전트 IDE로, AI 에이전트가 코드 수정·테스트를 자율 수행합니다:

1. 러버블에서 GitHub 연동으로 프로젝트 코드 내보내기
2. 안티그래비티에서 해당 폴더를 열고 워크스페이스 생성
3. 에이전트에게 수정·기능 추가·테스트 작업을 지시

> **실사용자 경험**: "안티그래비티에서 에이전트에게 리팩토링 작업 시켜봤는데 신세계였음. 멀티 에이전트가 병렬로 코드 수정하니까 엄청 빠름" — 실사용자 경험. 러버블로 프로토타입 후 안티그래비티로 본격 개발·테스트까지 이어갈 수 있음
...
| 혜택 | 개발자 활용법 |
|------|---------------|
| **Free 플랜 크레딧(하루 5개)** | 가벼운 프로토타입 실험, 본격 개발은 Pro($25~) 권장 |
