...
> **주요 도구**: ChatGPT Plus/Pro, 코드 블록(구 Canvas), Codex, API
> **소요**: 약 4~5시간
...
- 코드 블록(구 Canvas)에서 코드를 수정해본 적이 있다
...
| **ChatGPT 릴리스 노트** | 코드 블록(구 Canvas)·기능 변경 안내 | [help.openai.com](https://help.openai.com/en/articles/6825453-chatgpt-release-notes) |
...
5. [코드 블록(구 Canvas) 심화 활용](#5-코드-블록구-canvas-심화-활용)
...
| **핵심 강점** | 범용 AI + 코드 블록 + Codex | 즉시 빌드+배포 | 에이전트 위임 | 코드 직접 제어 | 무료 프로토타입 | 터미널 파워 |
| **코드 관리** | 코드 블록 + Codex(GitHub) | GitHub 연동 | 내장 | 완전 제어 | 제한적 | Git 완전 통합 |
| **가격** | 무료~Pro ($100/$200), 진입 Go $8 | 유료 | 무료+유료 | 유료 | 무료 | API 종량제 |
...
| **코드 블록(구 Canvas)** | 채팅 응답 안에서 코드를 직접 편집·실행·미리보기하는 통합 코드 블록 (2026년 5월 Canvas가 이 기능으로 통합됨) |
...
| 한계 | 보완 방법 |
|------|----------|
| 프로젝트 파일을 직접 관리하지 않음 | Codex로 GitHub 연동, 또는 코드 블록에서 코드 내보내기 |
| 빌드/실행 환경이 제한적 | Codex 샌드박스 또는 로컬 환경에서 실행 |
| 긴 대화에서 컨텍스트 유실 | 시스템 프롬프트 활용, 대화 분기 전략 |
| 실시간 미리보기 없음 (코드 블록 제외) | 로컬에서 직접 실행하거나 러버블/AI Studio 활용 |
...
- [ ] ChatGPT의 강점(코드 블록, Codex, GPTs)을 이해했다
...
### GPT-5.5 Instant · Thinking · Pro의 차이

2026년 6월 현재 ChatGPT의 기본 모델은 **GPT-5.5 세대**입니다. 과거 교안의 GPT-4o / o3 대신 GPT-5.5(Instant / Thinking / Pro)를 상황에 맞게 전환하면 효율적입니다. (GPT-5.4 계열은 보조로 공존)

| 항목 | **GPT-5.5 Instant** | **GPT-5.5 Thinking** | **GPT-5.5 Pro** |
|------|---------------------|----------------------|-----------------|
| **속도** | 빠름 | 느림 (심층 추론) | 가장 느림 (최고 성능) |
| **강점** | 일반 코딩, 빠른 반복 | 복잡한 로직, 수학, 알고리즘 | 고난도 문제, 장문 설계 |
| **비용/티어** | 무료 티어 기본 | Plus 이상 | Pro 티어 ($100/$200) |
| **추천 용도** | UI 컴포넌트, CRUD, 스타일링 | 아키텍처 설계, 복잡한 버그 디버깅, 최적화 | 대규모 리팩터, 심층 연구성 작업 |
| **코드 블록 지원** | 지원 | 지원 | 지원 |

```
💡 모델 전환 전략 (2026년 6월 기준):
- 일반 코딩/반복 작업: GPT-5.5 Instant (빠르고 효율적, 무료 기본)
- 복잡한 설계/디버깅: GPT-5.5 Thinking (깊은 추론)
- 고난도/전문 업무: GPT-5.5 Pro (Pro 티어)
- 간단한 질문/포맷팅·대량 처리: GPT-5.4 mini (빠르고 저렴)
```
...
3. 같은 요청을 GPT-5.5 Instant와 GPT-5.5 Thinking으로 각각 보내고 결과를 비교해보세요

### ✅ 체크포인트

- [ ] 프롬프트 체이닝 전략을 3단계 이상으로 설계할 수 있다
- [ ] 커스텀 지침을 프로젝트에 맞게 설정할 수 있다
- [ ] GPT-5.5 Instant와 Thinking을 상황에 맞게 전환할 수 있다
- [ ] 대화 분기와 재시도를 효과적으로 사용할 수 있다
...
### 구독 플랜별 기능 비교 (2026년 6월 기준, 변동 가능)

기존 Free/Plus/Pro 구조에 저가 **Go($8)** 티어와 **Pro $100** 중간 티어가 더해져 6티어 체계로 세분화되었습니다.

| 기능 | **Free** | **Go ($8)** | **Plus ($20)** | **Pro ($100/$200)** |
|------|----------|-------------|----------------|---------------------|
| GPT-5.5 Instant | 기본 (한도 有) | 확대 | 무제한 수준 | 무제한 수준 |
| GPT-5.5 Thinking | 제한적 | 제한적 | 지원 | 지원 |
| GPT-5.5 Pro | 미지원 | 미지원 | 미지원 | 지원 |
| 코드 블록(구 Canvas) | 기본 | 전체 | 전체 | 전체 |
| Codex | 미지원 | 제한적 | 지원 | 고급 (사용량 대폭 증가) |
| GPTs 제작 | 미지원 | 지원 | 지원 | 지원 |
| API 크레딧 | 별도 구매 | 별도 구매 | 별도 구매 | 별도 구매 |
| 파일 업로드 | 제한적 | 지원 | 무제한 | 무제한 |
| 프로젝트 | 미지원 | 지원 | 지원 | 지원 |

> **참고**: 이 밖에 Business·Enterprise 티어가 있습니다. Codex는 별도 구독 없이 Plus 이상 플랜에 번들로 포함됩니다. 사용 한도(메시지·Codex 실행량)는 OpenAI가 수시 조정하므로 공식 가격 페이지를 확인하세요.
...
## 5. 코드 블록(구 Canvas) 심화 활용

> **바뀐 점 (2026년 5월 28일)**: 별도 화면이던 **Canvas는 종료**되었고, 그 기능이 채팅 응답 안의 **코드 블록(code block)**으로 통합되었습니다. 인라인 편집, 미리보기(HTML/React/SVG/Mermaid/Vega), Python 실행 등 Canvas가 하던 일은 코드 블록이 그대로 계승합니다. 다만 "Canvas"라는 별도 버튼·메뉴는 더 이상 없으므로, 아래 설명은 모두 **채팅 내 코드 블록** 기준입니다.

### 학습 목표

- 코드 블록에서 멀티 파일 프로젝트를 효율적으로 관리할 수 있다
- 코드 리팩토링, 테스트 코드 생성, 문서화를 자동화할 수 있다
- 코드 블록의 고급 기능(실행, 미리보기, 인라인 편집)을 활용할 수 있다

### 코드 블록 기본 이해

코드 블록은 ChatGPT 응답 안에서 코드를 작성·편집·실행할 수 있는 **통합 코드 영역**입니다. (2026년 5월 이전의 Canvas가 이 기능으로 흡수됨)

| 기능 | 설명 |
|------|------|
| **코드 작성** | 응답 안에 편집 가능한 코드 블록이 자동 생성 |
| **인라인 편집** | 블록 안에서 코드를 직접 클릭·수정 가능 |
| **실행** | Python 코드를 블록 내에서 실행하고 콘솔 출력 확인 |
| **미리보기** | HTML / React / SVG / Mermaid 다이어그램 / Vega 차트 렌더링 |
...
```
코드 블록이 생기는 방법 (2026년 6월 기준):
1. ChatGPT에 코드/긴 글 관련 요청을 하면 응답 안에 편집 가능한 코드 블록이 자동으로 생성됩니다
2. 별도의 "Canvas" 진입 버튼은 없어졌습니다 — 요청 내용에 따라 자동으로 나타납니다
3. HTML·React·Python 등을 요청하면 블록 우측에서 미리보기/실행을 지원합니다
```

### 멀티 파일 프로젝트 관리

코드 블록에서 여러 파일을 다루는 전략:

```
[프롬프트 — 프로젝트 구조 먼저 설계]
React 할 일 관리 앱의 파일 구조를 설계하고,
각 파일을 하나씩 코드 블록으로 작성해줘.
...
[다음 파일 요청]
App.tsx가 완성되었어.
다음 파일 TodoList.tsx를 코드 블록으로 작성해줘.
App.tsx에서 사용하는 props 구조에 맞춰줘.
```

### 코드 리팩토링

코드 블록에서 기존 코드를 개선하는 방법:
...
코드 블록은 Python 코드를 **직접 실행**하고 콘솔 출력을 보여줍니다. (HTML/React/SVG/Mermaid/Vega는 미리보기로 렌더링)

```python
# 코드 블록에서 실행 가능한 Python 예시
import json
...
| 팁 | 설명 |
|----|------|
| **부분 선택 편집** | 전체가 아닌 특정 부분만 선택하여 수정 요청 |
| **버전 비교** | 이전 버전과 현재 버전을 비교하여 변경사항 확인 |
| **복사하여 활용** | 코드 블록의 코드를 로컬 프로젝트에 복사하여 사용 |
| **주석으로 요청** | 코드 중간에 `// TODO: 여기에 에러 처리 추가` 형태로 요청 |
| **점진적 빌드** | 한 번에 전체를 요청하지 말고, 파일별/기능별로 점진적 구현 |

### 실습 과제

1. 코드 블록에서 간단한 React 컴포넌트(카운터)를 작성하고, 인라인 편집으로 기능을 추가해보세요
2. 작성한 컴포넌트에 대한 테스트 코드를 자동 생성해보세요
3. 코드 블록에서 Python 코드를 작성하고 실행해보세요

### ✅ 체크포인트

- [ ] 코드 블록에서 코드를 작성하고 직접 편집할 수 있다
- [ ] 멀티 파일을 순서대로 생성하는 전략을 사용할 수 있다
- [ ] 리팩토링, 테스트, 문서화를 코드 블록에서 수행할 수 있다
- [ ] 코드 블록에서 코드를 실행하고 결과(미리보기·콘솔)를 확인할 수 있다
...
| 파라미터 | 기본값 | 설명 | 추천 값 |
|----------|--------|------|---------|
| `model` | — | 사용할 모델 | `gpt-5.5`, `gpt-5.4-mini` (최신 모델명은 공식 문서 확인) |
...
|------|---------------------|---------------------|------|
| **gpt-5.5** | 공식 페이지 확인 | 공식 페이지 확인 | 일반 코딩·추론 (범용 기본) |
| **gpt-5.4-mini** | 공식 페이지 확인 | 공식 페이지 확인 | 간단한 작업·대량 처리 (경량·저비용) |

> **참고**: 2026년 6월 현재 API 모델은 GPT-5.5 / GPT-5.4 계열로 재편되었습니다. 정확한 모델 ID와 토큰당 단가는 수시로 바뀌므로, 위 표는 용도 구분만 참고하고 **최신 모델명·단가는 [platform.openai.com/pricing](https://platform.openai.com/pricing)에서 반드시 확인**하세요. (2026-07-04 기준)
...
response = client.chat.completions.create(
    model="gpt-5.4-mini",  # 간단한 분류, 포맷팅에 적합
...
3. gpt-5.5와 gpt-5.4-mini의 응답 품질과 비용을 비교해보세요
...
- 코드 블록에서 만든 코드를 로컬 프로젝트로 내보내는 방법을 이해한다
...
| 방법 | 설명 | 추천 상황 |
|------|------|----------|
| **코드 블록 → 복사** | 코드 블록에서 코드를 복사하여 로컬 파일에 붙여넣기 | 단일 파일, 빠른 테스트 |
...
4. GPT-5.5 Thinking은 더 긴 출력·심층 추론을 지원하므로 모델 전환 시도
```
...
#### 문제 4: "코드 블록에서 코드가 실행되지 않아요"

```
원인: 외부 패키지 의존성, 네트워크 접근 제한 등
(참고: 2026년 5월 Canvas가 코드 블록으로 통합됨 — 실행 제약은 유사)

해결 방법:
1. 코드 블록은 표준 라이브러리 위주 — 외부 패키지가 필요한 코드는 로컬에서 실행
2. API 호출 코드 — 로컬 환경에서 실행 (코드 블록에서는 네트워크 제한)
3. 파일 I/O — 코드 블록에서는 제한적, 로컬 환경 사용
4. "이 코드를 코드 블록에서 실행 가능하게 수정해줘" 요청
```
...
```
네, 무료 버전에서도 GPT-5.5 Instant(기본, 한도 有)를 사용하여 코드를 작성할 수 있습니다.
다만 GPT-5.5 Thinking, Codex, GPTs 제작 등 고급 기능은 Plus 이상에서 사용 가능합니다. (코드 블록은 무료에서도 기본 사용)
...
- gpt-5.4-mini로 간단한 작업: 월 $1~5
- gpt-5.5로 본격 개발: 월 $10~30
...
| 워크플로 | GPTs 구성 | 효과 |
|----------|-----------|------|
| **기획 → 구현** | 기획 도우미 GPT → ChatGPT 코드 블록 | 체계적인 코드 생성 |
...
요청 유형별 모델 분배:
━━━━━━━━━━━━━━━━━━━━━━━━━━
간단한 분류/포맷팅  →  gpt-5.4-mini  (경량·저비용)
일반 코딩/대화      →  gpt-5.5       (범용 기본)
복잡한 추론/설계    →  gpt-5.5 Thinking  (심층 추론)

예상 비용 절감: 50~70% (단가는 공식 가격 페이지 확인)
```
...
def cached_chat(messages: list, model: str = "gpt-5.5") -> str:
...
| 코드 블록 편집 | Cursor의 코드 에디터에서 더 강력하게 |
...
| **모델** | GPT-5.5 | Claude Opus 4.8·Sonnet 4.6 | Gemini 3.1 Pro·3.5 Flash |
