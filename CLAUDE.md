...
- 변경 내용에 따라 **CLAUDE.md**, **ia.md**, **erd.md**를 먼저 업데이트한다
- 새로운 페이지, 섹션, 데이터 구조가 추가되면 반드시 문서에 반영한다
- 문서가 현재 코드 상태와 항상 일치해야 한다

### 3단계: 코드 개발

- **MD 파일을 먼저 수정** → 이후 HTML에 반영 (MD가 원본)
- HTML은 MD 내용을 기반으로 수동 변환 (빌드 도구 없음)
- MD ↔ HTML 간 내용 동기화 필수
...
```
1. 계획하기 → 2. PRD/IA/ERD 반영 → 3. 코드 개발 → 4. 유효성 검증 → 5. 커밋/푸시/배포
```
