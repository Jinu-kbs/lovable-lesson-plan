...
6. [Replit 배포 — 가장 쉬운 올인원 배포](#6-replit-배포--가장-쉬운-올인원-배포)
7. [정적 사이트 vs 동적 앱 — 언제 백엔드가 필요할까](#7-정적-사이트-vs-동적-앱--언제-백엔드가-필요할까)
8. [도구별 배포 방법](#8-도구별-배포-방법)
9. [커스텀 도메인 연결하기](#9-커스텀-도메인-연결하기)
10. [배포 후 확인하기](#10-배포-후-확인하기)
...
> **핵심 포인트**: 이 교안에서 다루는 GitHub Pages, Vercel, Netlify 모두 **정적 사이트 배포에 최적화**된 플랫폼입니다. 바이브코딩으로 만든 프로젝트 대부분을 이 3가지 플랫폼으로 배포할 수 있습니다. 여기에 더해, **만들기부터 배포까지 한 탭에서 끝내는 올인원 플랫폼 Replit(리플릿)**도 6장에서 다룹니다.
...
| 만들기·실행·배포를 한 곳에서 끝내고 싶어요 | **Replit** | 코드 에디터 + 서버 + DB + 호스팅이 한 탭 안에 (6장) |
...
## 6. Replit 배포 — 가장 쉬운 올인원 배포

> 최종 검증 2026-06-28 · Replit은 화면(UI)·메뉴 이름·요금이 자주 바뀝니다. 버튼 이름이 교안과 달라 보이면 이름보다 "동선"을 기준으로 따라오세요.

### Replit이 "가장 쉬운" 이유

지금까지 본 GitHub Pages / Vercel / Netlify는 모두 **"내 컴퓨터에서 코드를 만들고 → 따로 가입한 배포 플랫폼에 올린다"**는 구조입니다. 즉 만드는 곳과 배포하는 곳이 분리되어 있습니다.

**Replit(리플릿)**은 이 분리를 없앤 **올인원(all-in-one) 플랫폼**입니다. 한 문장으로:

> **코드 에디터 + 서버 + 데이터베이스 + 호스팅이 브라우저 한 탭 안에 모두 들어 있어, 따로 가입할 서비스가 없습니다. 만들고, 그 자리에서 인터넷에 공개합니다.**

설치할 것이 없습니다. 회원가입 후 웹 에디터를 열면 그 자리에서 앱을 만들고, **Publish(게시) 버튼 한 번**으로 `*.replit.app` 공개 주소가 발급됩니다. 코드를 한 줄도 몰라도 **자연어로 "이런 앱 만들어 줘"** 하고 요청하면 Replit Agent(AI 빌더)가 앱을 만들어 주는 바이브코딩 흐름도 같은 화면 안에 있습니다.

| 구분 | GitHub Pages / Vercel / Netlify | **Replit** |
|------|--------------------------------|-----------|
| 만드는 곳 | 내 PC(에디터) 또는 바이브코딩 도구 | **브라우저 안 에디터(같은 곳)** |
| 배포하는 곳 | 따로 가입한 배포 플랫폼 | **같은 탭의 Publish 버튼** |
| 백엔드·DB | 없음(정적) 또는 외부 서비스 연동 | **내장 DB·서버 같이 구동** |
| AI 빌더 | 없음(도구 따로) | **Replit Agent 내장** |
| 첫 배포 체감 | 가입·연동 단계 있음 | **한 화면에서 끝** |

### 무료로 어디까지 되나 (정확히)

Replit 무료 플랜(Starter)으로도 **공개 앱을 배포할 수 있습니다.** 단, 아래 제약을 정확히 알아두세요.

- **무료 공개 앱은 1개**까지 운영할 수 있습니다.
- 무료 공개 배포는 **약 30일 후 만료**되며, 만료되면 **다시 게시(re-publish)**해야 유지됩니다.
- 무료 플랜의 배포 지역(리전)은 **북미 고정**(지역 선택 불가)입니다. 한국에서 학습·체험하기엔 문제없으나, 한국 사용자 대상 실서비스라면 약간의 접속 지연이 있을 수 있습니다.
- 커스텀 도메인은 **유료 플랜(Core 이상)**부터입니다. 무료는 `*.replit.app` 주소만 제공됩니다.
- 학습·체험용 **무료 AI 크레딧**이 매일 소량 제공됩니다.

> **정적 사이트는 호스팅 무료**: 단순 HTML/CSS/JS 같은 정적 사이트를 Replit의 **Static(정적)** 유형으로 올리면 호스팅 비용 자체가 무료입니다(데이터 전송량만 소량 과금). 포트폴리오·랜딩 페이지 같은 결과물은 사실상 추가비 없이 운영됩니다.

### 흔한 오해 정정 박스

> **오해 정정**
>
> **"Replit 앱은 5분 후 잠든다(슬립)?"** → 이 "5분 슬립"은 **개발용 작업공간(Repl) 한정**입니다. 즉 코드를 짜는 에디터가 5분간 활동이 없으면 잠드는 것이지, **공개 배포한 앱이 5분 후 죽는다는 뜻이 아닙니다.** 배포된 앱은 유형에 따라 다릅니다 — Static(정적)은 슬립 개념 자체가 없고(항상 응답), 24시간 무중단이 꼭 필요한 봇·백엔드는 Reserved VM(유료)으로 항상 켜 둡니다.
>
> **"비용은 작업(체크포인트)당 $0.25 정액?"** → **옛날 모델**입니다. 2026년 현재는 **노력 기반(Effort-Based) 과금** — 작업에 들인 시간·연산량에 비례해 **월 크레딧 안에서 차감**됩니다. 작은 변경은 더 싸게, 큰 작업은 그만큼 차감됩니다. (기존 유료 구독자에게는 2026년 7월 1일부터 순차 적용)

> **비용 안심 포인트**: 정적 사이트 호스팅은 무료, 가벼운 앱은 월 크레딧 안에서 사실상 추가비 없이 운영됩니다. "한 번 눌렀다고 큰돈이 빠져나가는" 구조가 아니니, 학습 단계에서는 마음 편히 실습하세요.

### Replit 5분 배포 핸즈온

빈 화면에서 **공개 URL까지** 5분 만에 가 봅니다. 화면이 교안과 조금 달라 보여도 "동선"은 같습니다.

**(1단계) 계정 만들기**

1. [replit.com](https://replit.com)에 접속해 무료 회원가입(Google 계정 등으로 가능).
2. 로그인하면 대시보드(내 프로젝트 목록)가 열립니다.

[스크린샷 자리: Replit 가입/대시보드 화면]

✅ **체크포인트**: 로그인된 상태에서 "Create"(만들기) 버튼이 보이는 대시보드가 나타났나요?

**(2단계) 앱 생성 — 템플릿으로 시작**

1. **Create**(또는 "+ Create Repl") 클릭.
2. **Template(템플릿)** 목록에서 만들고 싶은 종류를 고릅니다. 가장 쉬운 길은 **"HTML, CSS, JS"** 정적 템플릿입니다(순수 웹페이지).
   - 참고: 코드를 직접 안 짜고 싶다면 자연어로 요청하는 **Replit Agent**로 시작할 수도 있습니다("간단한 방명록 웹앱 만들어 줘"). 다만 첫 실습은 정적 템플릿이 가장 군더더기 없습니다.
3. 앱 이름을 입력하고 **Create** 클릭.

[스크린샷 자리: 템플릿 선택 화면]

✅ **체크포인트**: 가운데 코드 에디터, 오른쪽 미리보기(Webview) 영역이 함께 보이는 작업화면이 열렸나요?

**(3단계) 실행(Run)**

1. 상단의 **Run(실행)** 버튼을 누릅니다.
2. 오른쪽 미리보기에 페이지가 뜹니다. 이때 보이는 주소는 **임시 미리보기 URL**입니다 — 아직 진짜 공개 주소가 아닙니다.
3. (선택) `index.html`의 글자를 바꿔 보고 다시 미리보기에 반영되는지 확인.

[스크린샷 자리: Run 후 미리보기 화면]

✅ **체크포인트**: 오른쪽 미리보기에 내 페이지가 보이나요? 여기까지는 "내 작업공간에서만" 보이는 상태입니다.

**(4단계) Deploy(게시) — 공개 URL 발급**

1. 에디터 우상단의 **Publish(게시)** 버튼을 클릭합니다.
2. **배포 유형**을 고릅니다. Replit이 프로젝트 성격을 보고 추천 유형을 자동 제시합니다.
   - 정적 사이트(소개·포트폴리오)면 **Static(정적)**을 고르는 것이 가장 싸고 빠릅니다.
   - 트래픽이 들쭉날쭉한 일반 웹앱이면 **Autoscale**이 추천됩니다.
3. **Deploy** 클릭 → 잠시 빌드 후 공개됩니다.
4. 발급된 **`<앱이름>.replit.app` 주소**가 표시됩니다. 이 주소를 **휴대폰으로 열어** "내가 만든 앱이 인터넷에 떴다"를 체험해 보세요.

[스크린샷 자리: Publish 패널 + 배포 완료 후 공개 URL 화면]

> **주의 — 배포 실패 1위 원인**: 앱이 API 키 등 **비밀값(Secrets)**을 쓰는 경우, 개발 작업공간에 넣은 Secrets는 **배포로 자동 복사되지 않습니다.** 배포가 "undefined 값"으로 죽으면 십중팔구 이것 — 게시 화면(Deployments)에서 Secrets를 **따로 다시 입력**해야 합니다. (정적 HTML만 올릴 때는 해당 없음)

✅ **체크포인트**: `*.replit.app` 공개 주소가 발급되고, 휴대폰·다른 기기에서도 그 주소로 접속되나요? 축하합니다 — 올인원 배포 성공입니다!

### 더 배우려면

| 자료 | 용도 | 링크 |
|------|------|------|
| Replit 공식 문서 | 모든 기능의 1차 출처 | [docs.replit.com](https://docs.replit.com) |
| 배포 개요 | 배포 유형 선택 가이드 | [docs.replit.com/cloud-services/deployments/about-deployments](https://docs.replit.com/cloud-services/deployments/about-deployments) |
| 가격 페이지 | 결제 직전 정확 수치 재확인 | [replit.com/pricing](https://replit.com/pricing) |

---

## 7. 정적 사이트 vs 동적 앱 — 언제 백엔드가 필요할까

지금까지 배운 배포는 대부분 **정적 사이트(Static)** 배포였습니다. 그런데 사이트를 키우다 보면 "이건 파일만 올려서는 안 되는데?" 싶은 순간이 옵니다. 그 경계를 쉽게 짚어 봅니다.

### 한 줄 구분

- **정적 사이트**: 미리 만들어 둔 HTML/CSS/JS 파일을 **그대로 보여주기만** 하는 사이트. 누가 와도 똑같은 화면. (전단지에 비유)
- **동적 앱**: 사용자마다, 시간마다 **내용이 달라지는** 앱. 데이터를 **저장하고 꺼내 오는** 작업이 필요. (주문을 기억하는 카페 시스템에 비유)

### "그땐 백엔드/DB가 필요하다" — 신호 4가지

아래 중 **하나라도** 필요해지면, 정적 사이트만으로는 부족하고 **백엔드(서버) + 데이터베이스(DB)**가 필요해집니다.

| 하고 싶은 것 | 왜 정적 사이트로는 안 되나 |
|-------------|--------------------------|
| **로그인 / 회원가입** | 누가 가입했는지 안전하게 **기억(저장)**해야 함 |
| **글·댓글·예약을 저장** | 방문자가 입력한 내용을 **다음에도 남겨 둬야** 함 (새로고침해도 유지) |
| **사용자마다 다른 화면** | "내 정보", "내 주문"처럼 사람마다 다른 데이터를 꺼내 와야 함 |
| **비밀 키로 외부 서비스 호출** | API 키를 화면에 박으면 누구나 보임 → **서버 뒤에 숨겨야** 함 |

> **쉽게 기억하기**: "새로고침하면 사라지면 안 되는 데이터"가 생기는 순간이 곧 백엔드·DB가 필요해지는 순간입니다.

### 그럼 어떻게 하나 — 두 갈래 길

1. **올인원으로 한 곳에서 끝내기 → Replit (6장)**: Replit은 내장 DB와 서버를 같은 프로젝트에서 함께 구동하므로, "DB를 따로 가입 안 했는데도 글이 저장되는" 풀스택 앱을 한 탭에서 만들 수 있습니다. **가장 마찰이 적은 첫 풀스택 경로**입니다.

2. **전문 조합으로 가기 → Supabase + Vercel (중급편)**: 프론트엔드는 Vercel(빠른 CDN·자동 배포)에, 데이터 저장·로그인은 **Supabase**(관리형 PostgreSQL + 인증 + 스토리지)에 맡기는 구성입니다. 이식성·확장성이 좋아 **장기 운영**에 적합합니다.

> **다음 단계 안내**: 로그인·데이터 저장이 필요한 동적 앱의 배포는 [배포 중급자편](deployment-intermediate.html)에서 **Supabase + Vercel** 조합으로 자세히 다룹니다. 지금은 "이런 경계가 있구나"만 이해하면 충분합니다.

✅ **체크포인트**: 내가 만들 앱이 "정적"인지 "동적(백엔드 필요)"인지 구분되나요? 동적이라면 Replit(한 곳에서) 또는 Supabase+Vercel(중급편) 두 길이 있다는 것을 기억하세요!
