...
- [ ] Cursor 최신 버전(3.x 세대, 2026.4~) 설치 및 로그인
...
| 모델 | 특징 | 추천 용도 |
|------|------|----------|
| **Composer 2.5** (Cursor 자체 모델·Auto 기본) | 빠르고 저렴 | 대부분의 코딩 (기본 추천) |
| **Claude Sonnet 4.6** | 빠르고 정확 | 일반 코딩, 리팩토링 |
| **Claude Opus 4.8** | 복잡한 추론 | 아키텍처 설계, 대규모 변경 |
| **Gemini 3.1 Pro** | 긴 컨텍스트 | 대규모 코드 분석 |

> "일상 작업은 Composer 2.5(빠르고 저렴)로 처리하고, 복잡한 설계에만 Claude Opus 4.8을 쓰면 비용도 절약됩니다." — 실사용자 조언
