...
| 모델 | 특징 | 추천 용도 | 비용 |
|------|------|----------|------|
| **Claude Opus 4.8**(기본) | 복잡한 추론·장기 에이전트 코딩 | 아키텍처 설계, 대규모 리팩토링 (Claude Code 기본 모델) | 높음 |
| **Claude Sonnet 4.6**(보조) | 빠르고 정확 | 일반 코딩, 파일 편집 | 낮음 |
| **Claude Haiku 4.5**(보조) | 초고속 응답 | 코드 포맷팅, 간단한 질문 | 매우 낮음 |

> "기본 모델은 Opus 4.8이지만, 일반 코딩은 Sonnet 4.6으로 전환해 두면 비용을 50% 이상 절약할 수 있어요." — 실사용자 조언
...
`/model`로 Sonnet 4.6 전환 → `.claudeignore`에 `node_modules/`, `*.min.js` 추가
